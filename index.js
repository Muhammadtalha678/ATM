...
const pinAnswer = await inquirer.prompt({ message: "Enter Your Pin", type: "number", name: "pin" });
if (pinAnswer.pin === person.pin) {
    console.log(`Your Total Amount is ${person.amount}!!`);
    const choiceAnswer = await inquirer.prompt({ message: "What do you want to do?", type: "list", name: "options", choices: ['WithDraw', 'BalanceCheck'] });
    if (choiceAnswer.options === "WithDraw") {
        ...
        if (askMoney.withdraw === "1000") {
            ...
            const customMoney = await inquirer.prompt({ message: " How much money you want to withdraw?", type: "number", name: "customMoney" });
